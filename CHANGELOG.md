...
- 代理列表 API 支持协议/国家过滤后的服务端分页，WebUI 增加分页器与每页数量切换。
- 订阅刷新任务增加运行/验证/成功/失败状态跟踪，WebUI 订阅面板可显示最新刷新状态并在任务运行时加快轮询。
- 配置新增额外代理源和禁用源 URL，抓取器按运行时配置动态合并内置源与自定义源。
- 新增源状态 API 与源统计面板，支持查看代理源分组、启用状态、成功率和健康分。
...
- 继续扩展 `webui/server_api_test.go`，覆盖配置保存后 `sources/status` 接口对额外源、禁用源和运行时源状态的联动展示。
